...
export const Calendars = () => {
  ...
  return (
    <main className={cn("flex sm:flex-row flex-col items-center justify-center w-full m-0 md:p-6 py-8 lg:px-16")}>
      <section className={cn("relative flex flex-col md:w-fit w-full pt-6", !showForm && "md:w-full h-full md:max-w-[60rem]", showForm && "md:h-full")}>
        <header className={cn("fixed md:pb-10 px-6 sm:px-0", showForm && "px-0")}>
            ...
            <Timeline className="max-w-[20rem] w-full md:block hidden">
              {timelines.map((timeline, index) => (
                <TimelineItem key={index} status="done">
                  ...
                  <TimelineDot status={step > index ? "done" : step === index ? "current" : "default"} />
                  {index < timelines.length - 1 && <TimelineLine done={step > index} />}
                  <TimelineContent>
      ...
      {showForm &&
        <section className={cn("flex flex-col gap-4 p-4 w-full md:max-w-[60rem] h-full")}>
          <div className="dark:bg-neutral-800 py-6 rounded-lg h-fit sm:h-full">
            ...
            <Form {...form}>
              <form id="add-calendar" onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 px-[1.5rem] pb-[1.5rem] overflow-auto">
                {inputs[step].map((input, index) => {
                  ...
                  if (input.label === "") {
                    ...
                    return (
                      <div key={input.name} className="flex gap-4 items-end">
                        <FormField
                          control={form.control}
                          name={prev.name as keyof AddCalendarProps}
                          render={({ field }) => (
                            <FormItem className="flex-1">
                              <FormLabel className="text-left">{prev.label}</FormLabel>
                              <FormControl>
                                <PrevComponent
                                  className={cn("dark:focus:ring-skyblue dark:focus:ring-1 h-10 data-[state=closed]:!ring-0 data-[state=open]:ring-1 data-[state=open]:ring-skyblue transition-all duration-75 dark:!text-neutral-400", prev.className)}
                                  placeholder={prev.placeholder}
                                  mode="time"
                                  autoComplete="off"
                                  onChange={(e) => field.onChange(e instanceof Date ? e : e?.target.value)}
                                />
                              </FormControl>
                              <FormMessage />
                            </FormItem>
                        ...
                        <FormField
                          control={form.control}
                          name={input.name as keyof AddCalendarProps}
                          render={({ field }) => (
                            <FormItem className="flex-1">
                              <FormLabel className="text-left">{input.label}</FormLabel>
                              <FormControl>
                                <Component
                                  className={cn("dark:focus:ring-skyblue dark:focus:ring-1 h-10 data-[state=closed]:!ring-0 data-[state=open]:ring-1 data-[state=open]:ring-skyblue transition-all duration-75 dark:!text-neutral-400", input.className)}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem className="flex flex-col">
                          ...
                          <FormControl>
                            <Component
                              className={cn("dark:focus:ring-skyblue dark:focus:ring-1 h-10", input.className)}
                              placeholder={input.placeholder}
                              autoComplete="off"
                              onChange={(e) => field.onChange(e instanceof Date ? e : e?.target.value)}
                            />
              ...
              <footer className="flex w-full items-start justify-between px-[1.5rem]">
                <WootButton onClick={() => setStep((prevStep) => Math.min(prevStep + 1, inputs.length - 1))}>
                  Avançar
                </WootButton>
              </footer>
            </Form>
