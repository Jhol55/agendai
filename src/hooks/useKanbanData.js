...
import { useSearchParams } from 'next/navigation'; // Importação para Next.js
...
export function useDynamicKanbanData(reloadFlag = 0) {
  const searchParams = useSearchParams(); // Usando useSearchParams do Next.js
  ...
  useEffect(() => {
    // Extrai o parâmetro 'kbw' da URL usando useSearchParams
    ...
    if (lastParamRef.current === param && !reloadFlag) { // Added reloadFlag to prevent abort on manual reload
      console.log('[DEBUG] Parâmetro igual ao anterior e sem flag de recarregamento, efeito abortado');
      console.groupEnd();
      return;
    }
    ...
    fetchData();
  }, [searchParams, reloadFlag]); // Dependência mudou para searchParams.toString()
